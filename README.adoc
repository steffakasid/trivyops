...
## Variables:
  - `GITLAB_TOKEN`    - the GitLab token to access the Gitlab instance (only nneds read_api permissions)
...
`-o`, `[--output]` **string** Define how to output results [text, txt, table, tbl, json] (*default* "text")
...
The config file must be yaml format and can optionally be encrypthed using Sops. To
do so you can use the following Sops command: `sops --input-type yaml --output-type yaml ~/.trivyops`
...
LOG_LEVEL: warn
FILTER: ^group1\/(!subgroup)|(groupprefix.+)$
----

NOTE: All flags can also be set via config file. 

### Configpaths

- `${HOME}/.trivyops`
- `/etc/.trivyops`
- `${HOME}/.config/trivyops/.trivyops`
